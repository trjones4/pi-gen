...
do
	...
	then
		...
		then
			...
			do
				echo -n "$nic: "
				# TBJ - raspbian stretch ifconfig changed output format
				if ifconfig $nic &>/dev/null && ifconfig $nic | grep 'inet' &>/dev/null
				then
					ifconfig $nic | grep 'inet' | sed -e 's/ B.*//' -e 's/.*://'
				else
					echo $"not connected"
				fi
			done
...
#	TBJ - removing software update function to prevent updates until new process developed
#			if [[ "$NON_NATIVE" == "" ]]
#			then
...
# TBJ - removing git based software update until new process developed
...
#				exit 0
	...
	# TBJ - user request to increase delay time, was 2 seconds, now 5	
	echo "Returning to mission menu in 5 seconds ..."
	sleep 5
done
